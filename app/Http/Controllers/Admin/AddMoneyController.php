...
class AddMoneyController extends Controller
{
    ...
    public function canceled()
    {
        ...
        $transactions = Transaction::with(
            'user:id,firstname,lastname,email,username,mobile',
            'gateway_currency:id,name',
        )->where('type', 'ADD-MONEY')->where('status', 0)->latest()->paginate(20);
        return view('admin.sections.add-money.index', compact(
            'page_title',
            'transactions'
        ));
    ...
    public function approved(Request $request){ 
        ...
        try{
            //update wallet
            $userWallet = UserWallet::find($data->wallet_id);
            ...
            UserNotification::create([
                'type'    => NotificationConst::PAYMENT_APPREVED,
                'user_id' => $user->id,
    ...
    public function rejected(Request $request){ 
        ...
        try{
            ...
            UserNotification::create([
                'type'    => NotificationConst::PAYMENT_REJECTED,
                'user_id' => $user->id,
                'message' => $notification_content,
            ]);
            //Push Notifications 
