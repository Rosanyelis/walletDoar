...
class TransactionController extends Controller
{
    ...
    public function log(Request $request) { 
        $validator = Validator::make($request->all(),[
            'slug'      => "nullable|string|in:add-money,money-transfer,money-exchange,withdraw,make-payment",
        ]);
        ...
        try{ 
            if(isset($validated['slug']) && $validated['slug'] != "") {
                ...
                $query->where(function ($q) {
                    $q->where('user_type', GlobalConst::USER)
                    ->where('receiver_id', auth()->id());
                })
                ->orWhere('user_id', auth()->id());
            })->where("type",$this->slugValue($validated['slug']))->orderByDesc("id")->paginate(20);
            }else {
                $transactions = Transaction::with('user','receiver_info')->where(function ($query) {
                $query->where('user_type', GlobalConst::USER)
                    ->where('receiver_id', auth()->user()->id);
                })->orWhere('user_id', auth()->user()->id)->orderByDesc("id")->paginate(20);
            }  
        ...
        return Response::success([__('Transactions fetch successfully!')],[
            'instructions'  => [
                'slug'      => "add-money,money-transfer,withdraw,money-exchange,make-payment",
