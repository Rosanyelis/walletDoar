...
class TransactionController extends Controller
{
    ...
    public function index($slug = null) {
        if($slug != null){
            ...
            ->where('type', $this->slugValue($slug))
            ->orderByDesc('id')
            ->paginate(20);
            ...
            })->orWhere('user_id', auth()->user()->id)->orderByDesc("id")->paginate(12);
    ...
    public function search(Request $request) {
        ...
        try{
            ...
            $query->where('user_type', GlobalConst::USER)
                  ->where('receiver_id', auth()->user()->id);
            })->orWhere('user_id', auth()->user()->id)->where("type",$this->slugValue($request->type))->search($validated['text'])->take(10)->get();
        }catch(Exception $e){
