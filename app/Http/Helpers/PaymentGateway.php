...
class PaymentGateway {
    ...
    use Paypal, Gpay, CoinGate, QRPay, Tatum, Stripe, Flutterwave, SslCommerz, Razorpay, PerfectMoney, PaystackTrait, Authorize;

    protected $request_data;
    protected $output;
    protected $currency_input_name      = "currency"; // funding payment gateway currency
    protected $wallet_currency_input    = "wallet_currency"; // user wallet currency
    ...
    public function gateway() {
        ...
        if($this->project_currency == PaymentGatewayConst::PROJECT_CURRENCY_SINGLE) {
            ...
            $this->output['wallet']             = $this->getUserWallet($validated[$this->wallet_currency_input], PaymentGatewayConst::WALLET_CURRENCY);
        }
    ...
    public function getUserWallet($currency, $currency_type = PaymentGatewayConst::GATEWAY_CURRENCY) {
        ...
        if($currency_type == PaymentGatewayConst::GATEWAY_CURRENCY) {
            $user_wallet = $wallet_model::auth()->whereHas("currency",function($q) use ($currency){
                $q->where("code",$currency->currency_code);
            })->first();
        }else if($currency_type == PaymentGatewayConst::WALLET_CURRENCY) {
            $currency_code = $currency;
            $user_wallet = $wallet_model::with('currency')->auth()->whereHas("currency",function($q) use ($currency_code){
                $q->where("code",$currency_code);
            })->first();
        }
    ...
    public function validator($data) {
        $validator = Validator::make($data,[
            ...
            $this->wallet_currency_input    => 'nullable|string',
        ]);
    ...
    public function chargeCalculate($currency,$receiver_currency = null) {  
        ...
        if($receiver_currency) {
        ...
        }else if($this->project_currency == PaymentGatewayConst::PROJECT_CURRENCY_MULTIPLE){ // if project currency multiple
            $receive_wallet     = $this->output['wallet'];
            $exchange_rate      = (1 / $receive_wallet->currency->rate) * $currency->rate;
            $will_get           = $amount; 
            $fixed_charge_calc      = $fixed_charges;
            $percent_charge_calc    = (($amount / 100 ) * $percent_charges) * $exchange_rate;
            $total_charge           = $fixed_charge_calc + $percent_charge_calc;
            ...
            $data = [
                ...
                'sender_cur_code'           => $currency->currency_code,
                'sender_cur_rate'           => $sender_currency_rate ?? 0,
                'receiver_cur_code'         => $receive_wallet->currency->code,
                'receiver_cur_rate'         => $receive_wallet->currency->rate ?? 0,
                'fixed_charge'              => $fixed_charge_calc,
                'percent_charge'            => $percent_charge_calc,
                'total_charge'              => $total_charge,
    ...
    public function render() {
        ...
        $common_keys = ['gateway','currency','wallet_currency','amount','distribute'];
    ...
    public function responseReceive() {
        ...
        $register_wallets = PaymentGatewayConst::registerWallet();
        $wallet_model = $register_wallets[$tempData['data']->creator_guard];
        $creator_wallet = $wallet_model::find($tempData['data']->wallet_id);

        $validator_data = [
            $this->currency_input_name  => $gateway_currency->alias,
            $this->amount_input         => $requested_amount,
            $this->wallet_currency_input => $creator_wallet->currency->code,
        ];
    ...
    public function handleCallback($reference,$callback_data,$gateway_name) {
        ...
        if($transaction) {
            ...
            $validator_data = [
                $this->currency_input_name      => $gateway_currency->alias,
                $this->amount_input             => $requested_amount,
                $this->wallet_currency_input    => $user_wallet->currency->code,
            ];
