...
//Action Btn
$('.table-header-action-btn').on('click', function (e) {
  e.preventDefault();
  $('.action-btn-list').toggleClass('active');
});
$('.action-btn-list button').on('click', function () {
  const selectedText = $(this).text();
  $('.table-header-action-btn').html(selectedText + ' <i class="las la-angle-down"></i>');
  $('.action-btn-list').removeClass('active');
});
//sidebar Menu
...
//Role Permission
$(".dependency-check-group input[type=checkbox]").on('change', function() {
    if($(this).is(':checked')) {
      $(this).parents("tr").first().find("td .custom-check-group-wrapper input[type=checkbox]").prop("checked", true);
    }else {
      $(this).parents("tr").first().find("td .custom-check-group-wrapper input[type=checkbox]").prop("checked", false);
    }
});
$(".custom-check-group-wrapper input[type=checkbox]").on("change", function() {
    if($(this).is(":checked")) {
        let checkboxs = $(this).parents("tr").first().find("td .custom-check-group-wrapper input[type=checkbox]");
        let isFalse = false;
        ...
        if(isFalse) {
            $(this).parents("tr").first().find("td .dependency-check-group input[type=checkbox]").prop("checked", false);
        }else {
            $(this).parents("tr").first().find("td .dependency-check-group input[type=checkbox]").prop("checked", true);
        }
    }else {
        $(this).parents("tr").first().find("td .dependency-check-group input[type=checkbox]").prop("checked", false);
    }
...
setup_section_footer_social_link_input: `<div class="row">
<div class="col-xl-3 col-lg-3 form-group iconpicker-container">
...
<div class="col-xl-8 col-lg-8 form-group">
    ...
    <input type="text" placeholder="Type Here..." name="link[]" class="form--control  " value=""> 
</div>
<div class="col-xl-1 col-lg-1 form-group">
    <button type="button" class="custom-btn btn--base btn--danger row-cross-btn w-100"><i class="las la-times"></i></button>
</div>
