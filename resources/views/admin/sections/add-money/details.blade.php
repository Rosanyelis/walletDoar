            ...
            <div class="row align-items-center mb-10-none">
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list-two">
                        ...
                        <li class="three">Full Name: <span>
                            @if($data->user_id != null)
                                    <a href="{{ setRoute('admin.users.details',$data->user->username) }}">{{ $data->user->fullname }} ({{ __("USER") }})</a>  
                            @endif
                            </span></li>
                        <li class="four">Method: <span>{{ @$data->gateway_currency->name }}</span></li>
                ...
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list two">
                        <li class="one">Fees & Charges: <span>{{ number_format(@$data->total_charge,2) }} {{ @$data->gateway_currency->currency_code }}</span></li>
                        <li class="two">Conversion Amount: <span>{{ number_format(@$data->request_amount*$data->exchange_rate,2) }} {{ @$data->gateway_currency->currency_code }}</span></li>
                        <li class="three">Rate: <span>1 {{ $data->request_currency }} = {{ number_format(@$data->exchange_rate,4) }} {{ @$data->gateway_currency->currency_code }}</span></li>
                        <li class="four">Payable: <span>{{ number_format(@$data->total_payable,2) }} {{ @$data->gateway_currency->currency_code }}</span></li>
...
@if ($data->gateway_currency->gateway->type == App\Constants\PaymentGatewayConst::MANUAL) 
                ...
                @if (@$item->type == "file")
                    ...
                    @else
                        <span class="text--danger">
                            ...
                            @endphp
                            <a href="{{ setRoute('file.download',["transaction",$item->value]) }}" >
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header p-3" id="approvedModalLabel">
                <h5 class="modal-title">{{ __("Approved Confirmation") }} ( <span class="fw-bold text-danger">{{ get_amount(@$data->request_amount,$data->request_currency) }}</span> )</h5>
                ...
                <form class="modal-form" action="{{ setRoute('admin.add.money.approved') }}" method="POST">
                    ...
                    <div class="row mb-10-none">
                        <div class="col-xl-12 col-lg-12 form-group">
                           ...
                           <p>{{ __("Are you sure to approved this reques") }}t?</p>
                        </div>
                    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header p-3" id="rejectModalLabel">
                <h5 class="modal-title">{{ __("Rejection Confirmation") }} ( <span class="fw-bold text-danger">{{ get_amount(@$data->request_amount,$data->request_currency) }}</span> )</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
