...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#app-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i>
                        {{ __('Add Item ') }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                <td>
                                    <ul class="user-list">
                                        <li><img src="{{ get_image($item->image ?? '', 'site-section') }}" alt="image">
                                        </li>
                                    </ul>
                                </td>
                                <td>{{ $item->language->$system_default_lang->title ?? '' }}</i></td>
                                <td>{{ $item->language->$system_default_lang->sub_title ?? '' }}</i></td>
                                <td>{{ $item->link ?? '' }}</i></td>
    ...
    @include('admin.components.modals.site-section.add-app-item')

    {{-- Solution Item Edit Modal --}}
    <div id="app-edit" class="mfp-hide large">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif"
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                ...
                                                'value' => old(
                                                    $lang_code . '_sub_title_edit',
                                                    $data->value->language->$lang_code->sub_title ?? ''),
                                            ])
                                        </div>
                                    </div>
                                @endforeach
                                <div class="form-group">
                                    @include('admin.components.form.input', [
                                        'label' => 'Link*',
                                        'name' => 'link_edit',
                                        'value' => old('icon', $data->value->link ?? ''),
                                        'class' => 'form--control',
                                    ])
                                </div>
                            </div>
...
@push('script')
    <script>
        openModalWhenError("app-add", "#app-add");
        openModalWhenError("app-edit", "#app-edit");
        ...
        $(".edit-modal-button").click(function() {
            ...
            $.each(languages, function(index, item) {
                editModal.find("input[name=" + item.code + "_title_edit]").val(oldData.language[item.code]
                    ?.title);
                editModal.find("input[name=" + item.code + "_sub_title_edit]").val(oldData.language[item
                    .code]?.sub_title);
            });
            fileHolderPreviewReInit("#app-edit input[name=image_edit]");
            openModalBySelector("#app-edit");
        });
