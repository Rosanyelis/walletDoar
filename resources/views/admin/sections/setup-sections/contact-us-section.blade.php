...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#contact-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i>
                        {{ __('Add Item ') }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                <td>
                                    <ul class="user-list">
                                        <li><i class="{{ $item->icon ?? 'fas fa-check' }}"></i></li>
                                    </ul>
                                </td>
                                <td>{{ $item->language->$system_default_lang->title ?? '' }}</i></td>
                                <td>{{ $item->language->$system_default_lang->details ?? '' }}</i></td>
    ...
    @include('admin.components.modals.site-section.add-contact-item')

    {{-- Solution Item Edit Modal --}}
    <div id="contact-edit" class="mfp-hide large">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                ...
                                <div class="form-group">
                                    @include('admin.components.form.input', [
                                        'label' => 'Icon*',
                                        'name' => 'icon_edit',
                                        'value' => old('icon_edit', $data->value->icon ?? ''),
                                        'class' =>
...
@push('script')
    ...
    <script>
        openModalWhenError("contact-add", "#contact-add");
        openModalWhenError("contact-edit", "#contact-edit");
        ...
        $(".edit-modal-button").click(function() {
            ...
            var editModal = $("#contact-edit");

            editModal.find("form").first().find("input[name=target]").val(oldData.id);
            editModal.find("input[name=" + default_language + "_title_edit]").val(oldData.language[default_language]
                .title);
            editModal.find("textarea[name=" + default_language + "_details_edit]").val(oldData.language[
            ...
            $.each(languages, function(index, item) {
                editModal.find("input[name=" + item.code + "_title_edit]").val(oldData.language[item.code]
                    ?.title);
                editModal.find("textarea[name=" + item.code + "_details_edit]").val((oldData.language[item
                    .code] == undefined) ? '' : oldData.language[item.code].details);

            });

            openModalBySelector("#contact-edit");
