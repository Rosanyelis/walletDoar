...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#solution-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i>
                        {{ __('Add Solution') }}</a>
                </div>
            </div>
                        ...
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td><a href="{{ $item->language->$system_default_lang->link ?? '' }}"
                                        class="text--danger">{{ $item->language->$system_default_lang->title ?? '' }}</a>
                                </td>
                                <td><a href="{{ $item->language->$system_default_lang->link ?? '' }}"
                                        class="text--danger">{{ $item->language->$system_default_lang->link ?? '' }}</a>
                                </td>
                                <td>
    ...
    @include('admin.components.modals.site-section.add-solution-item')

    {{-- Solution Item Edit Modal --}}
    <div id="solution-edit" class="mfp-hide large">
                                ...
                                <div class="tab-pane @if (get_default_language_code() == language_const()::NOT_REMOVABLE) fade show active @endif"
                                    ...
                                    <div class="form-group">
                                        @include('admin.components.form.input', [
                                            ...
                                            'value' => old(
                                                $default_lang_code . '_social_icon_edit',
                                    ...
                                    <div class="form-group">
                                        @include('admin.components.form.input', [
                                            ...
                                            'value' => old(
                                                $default_lang_code . '_link_edit',
                                                $data->value->language->$default_lang_code->link ?? ''),
                                        ])
                                    </div>
                                </div>
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif"
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                ...
                                                'value' => old(
                                                    $lang_code . '_link_edit',
                                                    $data->value->language->$lang_code->link ?? ''),
                                            ])
                                        </div>
                                    </div>
                                @endforeach
...
@push('script')
    ...
    <script>
        openModalWhenError("solution-add", "#solution-add");
        openModalWhenError("solution-edit", "#solution-edit");
        ...
        $(".edit-modal-button").click(function() {
            ...
            var editModal = $("#solution-edit");

            console.log(oldData);

            editModal.find("form").first().find("input[name=target]").val(oldData.id);
            editModal.find("input[name=" + default_language + "_title_edit]").val(oldData.language[default_language]
                .title);
            editModal.find("input[name=" + default_language + "_social_icon_edit]").val(oldData.language[
                default_language].social_icon);
            editModal.find("input[name=" + default_language + "_link_edit]").val(oldData.language[default_language]
                .link);

            $.each(languages, function(index, item) {
                editModal.find("input[name=" + item.code + "_title_edit]").val(oldData.language[item.code]
                    .title);
                editModal.find("input[name=" + item.code + "_social_icon_edit]").val(oldData.language[item
                    .code].social_icon);
                editModal.find("input[name=" + item.code + "_link_edit]").val(oldData.language[item.code]
                    .link);
            });
            editModal.find("input[name=image]").attr("data-preview-name", oldData.image);
            fileHolderPreviewReInit("#solution-edit input[name=image]");
            openModalBySelector("#solution-edit");
