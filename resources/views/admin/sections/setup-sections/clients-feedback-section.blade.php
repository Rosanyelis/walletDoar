        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#client-feedback-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i>
                        {{ __('Add Feedback') }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td><span title="{{ $item->language?->$system_default_lang?->comment ?? '' }}"
                                        class="text--dark">{{ Str::words($item->language?->$system_default_lang?->comment ?? '', 8, '...') }}</span>
                                </td>
                                <td>
    ...
    <script>
        ...
        $(".edit-modal-button").click(function() {
            ...
            $.each(languages, function(index, item) {
                editModal.find("textarea[name=" + item.code + "_comment_edit]").val(oldData.language[item
                    .code]?.comment);
            });
